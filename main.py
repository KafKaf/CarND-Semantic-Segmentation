...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # reduce number of filters to num_classes
    layer7_1x1_conv = tf.layers.conv2d(vgg_layer7_out, num_classes, 1, strides=(1, 1), padding='same'
                                , kernel_regularizer=tf.contrib.layers.l2_regularizer(L2R_SCALE))

    output = tf.layers.conv2d_transpose(layer7_1x1_conv, num_classes, 4, strides=(2, 2) , padding='same'
                                , kernel_regularizer=tf.contrib.layers.l2_regularizer(L2R_SCALE))

    layer4_out_scaled = tf.multiply(vgg_layer4_out, 0.01, name='layer4_out_scaled')
    layer4_1x1_conv = tf.layers.conv2d(layer4_out_scaled, num_classes, 1, strides=(1, 1), padding='same'
                                       , kernel_regularizer=tf.contrib.layers.l2_regularizer(L2R_SCALE))

    output = tf.add(output, layer4_1x1_conv)
    ...
    layer3_1x1_conv = tf.layers.conv2d(layer3_out_scaled, num_classes, 1, strides=(1, 1), padding='same'
                                       , kernel_regularizer=tf.contrib.layers.l2_regularizer(L2R_SCALE))

    output = tf.add(output, layer3_1x1_conv)
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    # Loss function with L2 Regularization with beta = L2R_BETA
    reg_losses = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
    loss = loss + L2R_BETA * sum(reg_losses)

    train_optimizer = tf.train.AdamOptimizer(learning_rate).minimize(loss)

    return logits, train_optimizer, loss
